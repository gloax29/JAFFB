...
import com.ffpm.sample.fileflat.file.readsimple.ConvertStringTableauString;
import com.ffpm.sample.fileflat.file.readsimple.NumberConvertDix0;

/**
 * @author Gloax29
 *
 */
public class LigneComplex{
	
	@PositionalMappingParse(offset = 1, length = 6)
	private String debutLigne ;
	//total 185 
	//format dix 0
	@PositionalMappingParse(offset = 7, length = 10)
	@PositionalJavaTypeAdapter(value=NumberConvertDix0.class)
	private Integer idLigne ;
	@PositionalMappingParse(offset = 17, length = 10,padding=PositionalMappingParse.TOP,stripChaine=true)
	private String colume ;
	@PositionalMappingParse(offset = 27, length = 8,required=true)
	@PositionalControlRegex(expression="([0-9]{2})([0-9_A-Z]{6})")
	private String refference ;
	//il faut converrtir en tableau
	@PositionalMappingParse(offset = 35, length = 97)
	@PositionalJavaTypeAdapter(value=ConvertStringTableauString.class)
	private String[] valueRef ;
	@PositionalMappingParse(offset = 132, length = 60)
	private String code ;

	/**
	 * @return the debutLigne
	 */
	public String getDebutLigne() {
		return debutLigne;
	}
	/**
	 * @param debutLigne the debutLigne to set
	 */
